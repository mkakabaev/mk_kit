...
class MKLayoutId<ID extends Object, DATA extends Object> extends ParentDataWidget<MKMultiChildLayoutParentData<ID, DATA>> {
    final ID id;
...
class _RenderLayout<ID extends Object, DATA extends Object> extends RenderBox
    with ContainerRenderObjectMixin<RenderBox, MKMultiChildLayoutParentData<ID, DATA>>, RenderBoxContainerDefaultsMixin<RenderBox, MKMultiChildLayoutParentData<ID, DATA>> {
    ...
    void setDelegate(MKMultiChildLayoutDelegate<ID, DATA> delegate) {
        if (this.delegate.shouldRelayout(delegate)) {
            ...
            while (child != null) {
                ...
                if (childParentData is MKMultiChildLayoutParentData<ID, DATA>) {
                    ...
                    if (layoutId != null) {
                        assert(() {
                            if (childMap.containsKey(layoutId)) {
                                throw FlutterError.fromParts(<DiagnosticsNode>[
                                    ...
                                    ErrorDescription('All MKLayoutId widget wrapper identifiers for a MKMultiChildLayout(delegate: $delegate) instance must be unique'),
                                ]);
                            }
                            return true;
                        }());
                        ...
                        assert(false, 'MKLayoutId widget wrapper must have a non-null id. Please check generic param ID and DATA types.');
                    }
                } else {
                    assert(false);
                    break;
